...
class EnhanceConfig:
    ...
    SUCCESS_FLAVORS = [
        "",                                           # 미사용 (인덱스 0)
        "첫 발을 뗐어요. 시작이 반입니다.",                  # → Lv.1
        "감 잡히기 시작했어요.",                            # → Lv.2
        "차트 읽는 눈이 트이고 있어요.",                      # → Lv.3
        "슬슬 초보 딱지 뗄 때가 됐죠.",                      # → Lv.4
        "여기까지 올라온 것만으로 상위권이에요.",                # → Lv.5
        "이제부터 실패하면 처음부터예요. 각오하세요.",            # → Lv.6
        "한 번 실패하면 전부 날아가는 구간... 멋져요.",          # → Lv.7
        "이 정도면 배짱이 두둑하시네요.",                      # → Lv.8
        "열 명 중 다섯은 여기서 탈락합니다.",                   # → Lv.9
        "두 자릿수 진입. 이건 진짜 실력이에요.",                # → Lv.10
        "성공률 38%를 뚫다니... 존경합니다.",                  # → Lv.11
        "여기서부터는 운이 아니라 근성이에요.",                  # → Lv.12
        "성공률 20%대, 뚫는 사람이 소수예요.",                  # → Lv.13
        "이 톡방에서 Lv.14는 거의 전설이에요.",                # → Lv.14
        "성공률 18%... 심장 멀쩡하세요?",                    # → Lv.15
        "성공률 14%를 뚫는 사람은 인간이 아니에요.",             # → Lv.16
        "성공률 11%... 이 톡방 역대급 기록이에요.",             # → Lv.17
        "성공률 8%를 뚫었어요. 할 말을 잃었습니다.",             # → Lv.18
        "성공률 6%... 실화입니까?!",                        # → Lv.19
        "4%를 뚫고 만렙 달성! 이 톡방의 전설로 기록됩니다.",      # → Lv.20
    ]

    # 레벨별 실패 시 문구 (시도 레벨 기준, 0→1 ~ 19→20)
    FAIL_FLAVORS = [
        "95%에서 빠졌어요... 로또 사세요 (반대로).",            # Lv.0→1 실패
        "90%도 안 되는 거였어요?!",                          # Lv.1→2 실패
        "85%에서 떨어지면 좀 억울하죠.",                       # Lv.2→3 실패
        "80%면 거의 된 거였는데...",                          # Lv.3→4 실패
        "75%에서 미끄러졌어요. 괜찮아요, 다시 가죠.",            # Lv.4→5 실패
        "65%... 낮은 확률은 아니었는데.",                      # Lv.5→6 실패
        "60%에서 빠지면 좀 아프죠.",                          # Lv.6→7 실패
        "55%, 거의 반반이었는데 운이 안 따랐어요.",              # Lv.7→8 실패
        "50%... 동전 던지기에서 졌네요.",                      # Lv.8→9 실패
        "45%... 사실 실패가 더 높긴 했어요.",                  # Lv.9→10 실패
        "38%... 원래 어려운 구간이에요.",                      # Lv.10→11 실패
        "32%의 벽은 높았어요.",                              # Lv.11→12 실패
        "26%... 이 구간은 원래 안 되는 게 정상이에요.",          # Lv.12→13 실패
        "22%... 도전 자체가 대단한 거예요.",                   # Lv.13→14 실패
        "18%... 실패가 당연한 확률이에요. 괜찮아요.",            # Lv.14→15 실패
        "14%... 이건 진짜 간 큰 도전이었어요.",                 # Lv.15→16 실패
        "11%... 도전 자체를 존경합니다.",                      # Lv.16→17 실패
        "8%에 배팅하다니... 다음엔 될 거예요.",                 # Lv.17→18 실패
        "6%... 이 확률에 도전하는 것 자체가 전설이에요.",          # Lv.18→19 실패
        "4%... 여기까지 온 것만으로 이미 레전드예요.",             # Lv.19→20 실패
    ]
    ...
    # 각성 단계 칭호 (레벨별 개별 후보군) — 투자 커리어 성장 테마
    # 각 레벨마다 후보 칭호 3개, 게임 중 랜덤 표시
    TITLE_NAMES = {
        0:  [("예비 투자자", "🔰"), ("주린이", "🔰"), ("증시 새내기", "🔰")],
        1:  [("시장 탐험가", "🗺️"), ("주식 견습생", "📋"), ("데이터 수집가", "📡")],
        2:  [("차트 견습생", "📉"), ("지표 추적자", "🔎"), ("캔들 관찰자", "🕯️")],
        3:  [("주식 분석 수련생", "📚"), ("시황 독해사", "📰"), ("재무제표 탐독가", "📄")],
        4:  [("차트 분석가", "📈"), ("보조지표 해독사", "🔬"), ("기술적 분석 입문자", "💹")],
        5:  [("섹터 분석가", "🏭"), ("산업 연구가", "🔍"), ("테마 투자 전략가", "🧩")],
        6:  [("스윙 트레이더", "🎯"), ("포지션 설계자", "🗂️"), ("타이밍 조율사", "⏱️")],
        7:  [("포트폴리오 설계자", "💼"), ("자산배분 전략가", "⚖️"), ("분산투자 기획자", "🎲")],
        8:  [("수급 분석가", "🔄"), ("시장 심리 독해사", "🧠"), ("거래량 해석가", "📊")],
        9:  [("고급 차트 전문가", "📐"), ("패턴 마스터", "🎨"), ("추세 예측가", "🔭")],
        10: [("시장 해석가", "🌐"), ("마켓 사이클 분석가", "🔄"), ("매크로 투자가", "🌍")],
        11: [("알파 헌터", "🦅"), ("초과수익 전략가", "💡"), ("팩터 투자자", "⚗️")],
        12: [("리스크 설계자", "🛡️"), ("헤지 전략가", "🎭"), ("변동성 관리자", "🌪️")],
        13: [("리스크 매니저", "🧩"), ("포트폴리오 리밸런서", "⚖️"), ("손실 제어 전문가", "🔧")],
        14: [("시스템 트레이더", "🤖"), ("알고리즘 설계자", "💻"), ("규칙 기반 투자자", "📏")],
        15: [("퀀트 애널리스트", "🔢"), ("계량 분석가", "🧮"), ("데이터 과학 투자자", "🧪")],
        16: [("퀀트 전략가", "⚡"), ("알파 팩터 엔지니어", "🔩"), ("시장 모델링 전문가", "📡")],
        17: [("마켓 위저드", "🌟"), ("시장 지배자", "🏹"), ("수익 극대화 전문가", "💎")],
        18: [("투자 그랜드마스터", "🏆"), ("부의 설계자", "🏛️"), ("시장 정복자", "⚔️")],
        19: [("마켓 메이커", "🔮"), ("유동성 공급자", "💧"), ("시장 창조자", "✨")],
        20: [("투자의 신", "👑"), ("시장의 지배자", "🌈"), ("돈의 군주", "💫")],
    }
