...
실패 시 레벨과 직군·종이 모두 초기화된다.

각성에는 시간 제한이 없다. 예전에는 장 마감 후에만 되도록 막았는데,
각성은 시세와 무관한 성장 요소라 장중에 막을 이유가 없고 낮에 들어온
유저가 할 게 없어진다. 유일한 관문은 비용이고 그 돈은 주식으로 번다.
...
class EnhanceConfig:
    ...
    # 직군과 종도 같이 풀려서 다음에 Lv.10에 도달할 때 새로 뽑는다.
    ...
    # 직군 배정 레벨 (이 레벨에 도달하면 40직군 중 하나가 랜덤 배정된다)
    ...
    @classmethod
    def get_title(cls, level: int, seed: Optional[int] = None) -> Tuple[str, str]:
        """레벨에 해당하는 칭호와 이모지.

        예전에는 3계열(트레이더/투자가/퀀트)마다 Lv.10~19 전용 칭호 트리가
        따로 있었다. 지금은 40직군 도감이 그 역할을 하므로 칭호는 레벨만
        본다. 정체성은 직군 이름(⚡ 스캘퍼)과 종이 대신 표현한다.
