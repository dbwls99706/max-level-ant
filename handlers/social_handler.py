...
class SocialHandlerMixin(BaseHandlerMixin):
    ...
    def handle_ranking(self) -> Dict:
        ...
        if my_rank_in_top:
            ...
            my_rank = RankingService.get_my_group_rank(
                self.db, self.kakao_id, self.group_key
            )
            if my_rank:
    ...
    def _handle_ranking_solo(self) -> Dict:
        ...
        amount_str = (
            f"+{profit_amount:,}원" if profit_amount >= 0 else f"{profit_amount:,}원"
        )
...
💡 그룹 채팅방에서 친구들과 랭킹을 겨뤄보세요!"""
    ...
    def handle_my_rank(self) -> Dict:
        ...
        if self.group_key:
            rank_info = RankingService.get_my_group_rank(
                self.db, self.kakao_id, self.group_key
            )
            scope = "이 방"
        else:
...
{motivation}{rival_line}"""
    ...
    def handle_enhance_ranking(self) -> Dict:
        ...
        rankings = RankingService.get_group_enhance_ranking(
            self.db, self.group_key, limit=10
        )
...
💡 그룹 채팅방에서 각성 랭킹을 겨뤄보세요!"""
    ...
    def handle_mission(self) -> Dict:
        ...
        # ── 주간 챌린지 ──
        try:
            ch_result = ChallengeService.get_user_challenge_progress(
            ...
            if ch_result["success"]:
                ...
                if pr["completed"]:
                    ch_status = (
                        "✅ 완료!" if pr["reward_claimed"] else "✅ 완료! (보상 대기)"
                    )
                else:
...
🗓️ 주간 챌린지: {ch["description"]}
   [{ch_bar}] {ch_status}  💰 {ch["reward"]:,}원"""
        ...
        next_ach = (
            status["available_achievements"][0]
        ...
        if len(parts) < 3:
            ...
            if top:
                battle_btn = {
                    "label": f"⚔️ {top} 상승",
                    "action": "message",
                    "messageText": f"/배틀 {top} 상승 {default_bet}",
                }
            else:
                battle_btn = {
                    "label": "📊 인기종목",
...
다음 도전자는?! ⚔️"""
    ...
    def handle_challenge(self) -> Dict:
        ...
        progress_bar = "▓" * int(pr["progress_rate"] / 10) + "░" * (
            10 - int(pr["progress_rate"] / 10)
        )
