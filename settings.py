...
class KISConfig:
    ...
    #
    # 8초 → 20초 → 40초로 올려 왔다. 실측이 계속 상한을 넘겼기 때문이다.
    # (응답 17.77초 / 상한 15초, 응답 18.21초 / 상한 15초 - 두 순위 모두)
    # 배경에서 한 번도 못 받으면 유저는 영영 빈 순위를 보므로, 기다릴 수
    # 있는 만큼 기다리는 게 맞다. 대가는 동시 호출 슬롯 1개를 그동안 잡는 것뿐.
    #
    # 주의: read 타임아웃은 '소켓 읽기 1회'마다 적용된다. 헤더를 받고 본문에서
    # 멈추면 전체 시간은 상한을 넘을 수 있다(실측 17.77초 > 상한 15초).
    # 이 값은 벽시계 상한이 아니라 '한 번의 정지를 얼마나 참을지'에 가깝다.
    REFRESH_TIMEOUT = float(os.getenv("KIS_REFRESH_TIMEOUT", "40.0"))

    # 연결(connect) 단계 타임아웃 상한 (초).
    ...
    # 순위 캐시 TTL (초). 이 시간이 지나야 요청 경로가 직접 KIS를 부른다.
    #
    # 갱신 주기보다 넉넉히 길어야 한다. 배경 갱신이 성공하면 그때마다 캐시가
    # 새로 채워지므로 TTL이 길다고 데이터가 낡지 않는다. 반대로 TTL이 한
    # 주기(느린 날은 갱신 2건 × 40초 + 45초)보다 짧으면 캐시가 비는 구간이
    # 생긴다. 그동안 유저는 fallback(직전 성공값)으로 응답받는다.
    RANK_CACHE_TTL = float(os.getenv("KIS_RANK_CACHE_TTL", "180"))
