...
class KISConfig:
    ...
    #
    # 8초로는 부족했다. 배포 후 실측에서 순위 응답이 8초 상한을 계속 넘겨
    # 캐시가 한 번도 안 찼다(연속 실패로 서킷까지 열렸다). 배경에서 못 받으면
    # 유저는 영영 빈 순위를 보므로, 기다릴 수 있는 만큼 기다리는 게 맞다.
    # 대가는 동시 호출 슬롯 1개를 그동안 잡고 있는 것뿐이다.
    REFRESH_TIMEOUT = float(os.getenv("KIS_REFRESH_TIMEOUT", "20.0"))
    ...
    # 주기(느린 날은 갱신 2건 × 20초 + 45초)보다 짧으면, 유저 요청이 만료된
    # 캐시를 만나 3.5초 예산 안에서 느린 KIS를 직접 부르다 실패한다.
    # 그건 배경 갱신을 만든 이유를 정면으로 되돌리는 일이다.
