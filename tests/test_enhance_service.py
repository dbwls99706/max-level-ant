...
class TestEnhanceAttempt:
    ...
    def test_enhance_fail_safe_zone(self, db, test_user):
        """안전 구간 (0-5) 실패 시 레벨 유지"""
        ...
        assert result["new_level"] == 3  # 레벨 유지
        assert result["drop"] == 0

    def test_enhance_fail_danger_zone_no_drop(self, db, test_user):
        """주의 구간 (6-10) 실패 시 레벨 유지 가능"""
        test_user.enhance_level = 8
        test_user.cash = 100_000_000
        db.commit()

        # 실패 (roll=100) + 하락 안 됨 (하락 roll=100, 30% 초과)
        with patch("services.enhance_service.random.randint", side_effect=[100, 100]):
            result = EnhanceService.attempt_enhance(db, test_user.kakao_id)

        assert result["enhanced"] is False
        assert result["new_level"] == 8  # 레벨 유지

    def test_enhance_fail_danger_zone_with_drop(self, db, test_user):
        """주의 구간 (6-10) 실패 시 레벨 하락"""
        test_user.enhance_level = 8
        test_user.cash = 100_000_000
        db.commit()

        # 실패 (roll=100) + 하락 (하락 roll=1, 30% 이내)
        with patch("services.enhance_service.random.randint", side_effect=[100, 1]):
            result = EnhanceService.attempt_enhance(db, test_user.kakao_id)
